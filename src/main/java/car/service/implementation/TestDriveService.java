package car.service.implementation;

import car.model.dao.implementation.TestDriveDAO;
import car.model.entity.TestDrive;
import car.service.AbstractService;


import java.sql.SQLException;
import java.util.List;

public class TestDriveService implements AbstractService<TestDrive> {

    private final TestDriveDAO dao = new TestDriveDAO();

    @Override
    public List<TestDrive> findAll() throws SQLException {
        return dao.findAll();
    }

    @Override
    public TestDrive findById(Integer id) throws SQLException {
        return dao.findById(id);
    }

    @Override
    public void create(TestDrive entity) throws SQLException {
        dao.create(entity);
    }

    @Override
    public void update(Integer id, TestDrive entity) throws SQLException {
        dao.update(id, entity);
    }

    @Override
    public void delete(Integer id) throws SQLException {
        dao.delete(id);
    }
}
